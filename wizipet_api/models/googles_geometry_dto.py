...
class GooglesGeometryDto(BaseModel):
    """
    GooglesGeometryDto
    """ # noqa: E501
    location: Optional[GooglesLatLngLiteralDto] = None
    viewport: Optional[GooglesBoundsDto] = None
    __properties: ClassVar[List[str]] = ["location", "viewport"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in GooglesGeometryDto) in the input: " + _key)

        _obj = cls.model_validate({
            "location": GooglesLatLngLiteralDto.from_dict(obj["location"]) if obj.get("location") is not None else None,
            "viewport": GooglesBoundsDto.from_dict(obj["viewport"]) if obj.get("viewport") is not None else None
        })
        return _obj
